...
The main functions are:
    ...
    - boxBackReach(f,X0,Y,rho): computes a covering of the parametric space.
        
    - buildSampler(descr): builds a sampler for each parameter.    
        
    - def estimateCDF(f,fIntv,X0,j,S,P,sl,enclR,res,B,rho,Xin): estimates the univariate CDF 
...
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.1,intvf=True) # covering computation
ns=ceiling(Nsample(sum([e[1] for e in Xout]),.001,.001/(len(P)+1))) # number of samples for epsilon=0.001 and delta=0.001
mu,hi,Pn,volN,_=estimateCDF(fAdv,fAdvI,rX0FN,param,[SFN],P,slFN,enclR=None,res=Xout,B=ns,rho=.1,Xin=Xin,intvf=True) #cdf estimation
...
def boxBackReach(f,X0,Y,rho=.1,intvf=True):   
    '''
...
def boxBackReachAlt(f,X0,Y,sl,rho=.1,intvf=True,N=None):  
    '''
...
def estimateCDF(f,fIntv,X0,j,S,P,sl,enclR=None,res=None,B=1,rho=.1,Xin=None,intvf=True):
    '''
    ...
    if type(res)==type(None):
        ...
        res=boxBackReach(fIntv,X0,S,rho=rho,intvf=intvf)
...
P=[7,8.8]  
Xin,Xout,vol = boxBackReachAlt(fAdvI,X0FN,SFN,sl=slFN,rho=.2,intvf=True)
rX0FN=refine(X0FN,P,param)
vol_out=sum([e[1] for e in Xout+Xin]) 
Ns=ceiling(Nsample(vol_out,.01,.001)) 
mu,hi,Pn,volN,_=estimateCDF(fAdv,fAdvI,rX0FN,param,[SFN],P,slFN,enclR=None,res=Xout+Xin,B=25600,rho=.1,Xin=[],intvf=True)
'''
...
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.1,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.001,.001/(len(P)+1)))
mu,hi,Pn,volN,_=estimateCDF(fAdv,fAdvI,rX0FN,param,[SFN],P,slFN,enclR=None,res=Xout,B=ns,rho=.1,Xin=Xin,intvf=True)
...
slFN=buildSampler(slDescrFN)
P=list(np.arange(7.5*10**(-4),10.5*10**(-4),.0000001))
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.1,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.001,.001/(len(P)+1)))
...
slDescrFN=[('uniform',(0,0.5)),('uniform',(0,0.5)), ('trunc_gauss',(0, dev,a_trunc,b_trunc)), ('trunc_gauss',(0, dev,a_trunc,b_trunc)),('trunc_gauss',(0, dev,a_trunc,b_trunc))]
slFN=buildSampler(slDescrFN)
P=list(np.arange(0,0.5,.01))
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.2,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.001,.001/(len(P)+1)))
mu,hi,Pn,volN,_=estimateCDF(fAdv,fAdvI,rX0FN,param,[SFN],P,slFN,enclR=None,res=Xout,B=ns,rho=.2,Xin=Xin,intvf=True)
plt.step(Pn,hi/mu,where='post')
plt.show()
cl,cu=cdfLU(hi,.001)
mump(cl,cu,Pn)
...
slDescrFN=[('uniform',(0,0.5)),('uniform',(0,0.5)), ('trunc_gauss',(0, dev,a_trunc,b_trunc)), ('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0, dev,a_trunc,b_trunc))]
slFN=buildSampler(slDescrFN)
P=list(np.arange(0,0.5,.001))
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.2,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.005,.005/(len(P)+1)))
mu,hi,Pn,volN,_=estimateCDF(fAdv,fAdvI,rX0FN,param,[SFN],P,slFN,enclR=None,res=Xout,B=ns,rho=.2,Xin=Xin,intvf=True)
plt.step(Pn,hi/mu,where='post')
plt.show()
cl,cu=cdfLU(hi,.01)
...
slFN=buildSampler(slDescrFN)
P=list(np.arange(0.05,0.12,0.001))
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.1,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.001,.001/(len(P)+1)))
...
slDescrFN=[('uniform',(0.04,0.08)),('uniform',(0.05,0.12)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)), ('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc)),('trunc_gauss',(0,dev,a_trunc,b_trunc))]
slFN=buildSampler(slDescrFN)
P=list(np.arange(0.04,0.08,0.001))
rX0FN=refine(X0FN,P,param)
Xin,Xout,vol = boxBackReachAlt(fAdvI,rX0FN,SFN,sl=slFN,rho=.1,intvf=True)
vol_out=sum([e[1] for e in Xout])
ns=ceiling(Nsample(vol_out,.001,.001/(len(P)+1)))
